...
=head1 SYNOPSIS
 ...
 vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,score2_name=score2_file.tsv.gz
 vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,multi=max
 vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,multi=average
 vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,exact_indel_match=false

=head1 DESCRIPTION

 A VEP plugin that retrieves pre-calculated scores for missense variants from tabix-indexed database files.
 ...
 chr, pos, ref, alt, gene, ref_amino_acid/alt_amino_acid, score
...
# to use with vep, copy to your plugins folder
 ...
 mv MiSc.pm ~/.vep/Plugins

# call vep directly using pre-calculated scores
 ./vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,score2_name=score2_file.tsv.gz
 ./vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,multi=max
 ./vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,multi=min
 ./vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,multi=average
 ./vep -i variations.vcf --plugin MiSc,score1_name=score1_file.tsv.gz,exact_indel_match=false

# the scores should have this structure: chr,pos,ref,alt,ens_gene_id,ref_aa/alt_aa,score
# amino acids should be single letter codes separated by /
...
my %INCLUDE_SO = map {$_ => 1} qw(missense_variant);
...
sub new {
  ...
  if (!keys %$params) {
    ...
    foreach my $param_name (keys %$params) {
      ...
      $self->{headers}->{$param_name} = "Pre-calculated missense deleteriousness score from $param_name database ($method_str when multiple scores exist, $match_str)";
    }
  }
...
sub run {
  ...
  # Run only for missense variants
  ...
  # Get amino acid change information - keep as complete string
  my $pep_allele_string = $tva->pep_allele_string;
  return {} unless $pep_allele_string && $pep_allele_string =~ /\//;
  ...
  foreach my $data_record (@all_data) {
    # Match variant based on genomic coordinates, alleles, gene, and amino acid change
    if ($data_record->{pos} eq $vf->{start} &&
        $data_record->{ref} eq $seq_ref &&
        $data_record->{alt} eq $seq_alt &&
        $data_record->{gene} eq $gene_id &&
        $data_record->{aa_change} eq $pep_allele_string) {
...
sub parse_data {
  ...
  return {
    ...
    alt => $split[3],
    gene => $split[4],
    aa_change => $split[5], # Keep complete amino acid change string (ref_aa/alt_aa)
    score => $split[6],
