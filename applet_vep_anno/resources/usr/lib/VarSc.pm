...
=head1 SYNOPSIS
 ...
 vep -i variations.vcf --plugin VarSc,score1_name=score1_file.tsv.gz,score2_name=score2_file.tsv.gz
 vep -i variations.vcf --plugin VarSc,score1_name=score1_file.tsv.gz,multi=max
 vep -i variations.vcf --plugin VarSc,score1_name=score1_file.tsv.gz,multi=average
 vep -i variations.vcf --plugin VarSc,score1_name=score1_file.tsv.gz,exact_indel_match=false
...
# the scores should have this structure: chr,pos,ref,alt,score
# no gene or amino acid information required
...
sub run {
  ...
  # Run for all variants - no transcript/gene context needed
  ...
  foreach my $data_record (@all_data) {
    # Match variant based on genomic coordinates and alleles only (exact matching)
...
sub parse_data {
  ...
  # Ensure we have the expected number of columns (5 instead of 6)
  ...
  return {
    ...
    ref => $split[2],
    alt => $split[3],
    score => $split[4],  # Score is now in column 4 (0-indexed)
