check_ongoing_applet_runs() {
    ...
    log_message "INFO: Checking for current or previous runs with same input vcf list $VCF_LIST_HASH ..."
    ...
    while read -r job_id; do
        if [[ -n "$job_id" ]]; then
            ...
            if [[ -z "$job_describe" ]]; then
                ...
                job_input=$(echo "$job_describe" | jq -r '.runInput.vep_vcfs' 2>/dev/null || echo "")

                # If that didn't work or the result isn't as expected, fail
                if [[ -z "$job_input" ]] || [[ "$job_input" =~ ^null ]]; then
            ...
            if [[ "$job_input" == "${VCF_LIST_HASH}" ]]; then
                log_message "INFO: Found ongoing applet run with same input input file $VCF_LIST_NAME:"
